...
from sbi.inference.posteriors.mcmc_posterior import _maybe_use_dict_entry
...
# Adapt the posterior in a way that experimental conditions can be passed:
# add self.potential_fn.set_exp_cond to the sample and log prob functions
# Add max_width as a parameter for the SliceSampler.
class MCMCPosterior(NeuralPosterior):
    ...
    def potential(
        ...
        if exp_cond is not None:
            self.potential_fn.set_exp_cond(exp_cond)
    ...
    def log_prob(
        ...
        Args:
            theta: Parameters $\theta$.
            exp_cond: Experimental conditions.
            track_gradients: Whether the returned tensor supports tracking gradients.
                This can be helpful for e.g. sensitivity analysis, but increases memory
                consumption.
        Returns:
